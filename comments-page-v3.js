document.addEventListener('DOMContentLoaded', () => {
    const fallbackComments = [
        { full_name: 'Mert A.', location: 'Süleymanpaşa', rating: 5, body: 'Arsamız için yaptığımız ilk görüşmede fizibiliteyi ve olası proje senaryolarını çok net anlattılar. Süreç yaklaşımı güven verdi.' },
        { full_name: 'Selin K.', location: 'Tekirdağ', rating: 5, body: 'Ofis yenileme projesinde ihtiyaçlarımızı doğru okudular. Malzeme ve uygulama kararları beklediğimizden daha derli toplu ilerledi.' },
        { full_name: 'Kaan D.', location: 'Altınova', rating: 4, body: 'Cephe ve iç mekân detaylarını yerinde görmek karar vermemizi kolaylaştırdı.', image_url: 'https://cdn.lavira360.com/pioneer/gorseller/WhatsApp%20Image%202026-08-20%20at%2022.24.24%20(2).jpeg' },
        { full_name: 'Ece T.', location: 'Değirmenaltı', rating: 5, body: 'Villa projemiz için planlama aşamasında sundukları alternatifler sayesinde arsayı çok daha doğru değerlendirme şansı bulduk.', image_url: 'https://cdn.lavira360.com/pioneer/gorseller/WhatsApp%20Image%202026-08-20%20at%2022.24.26%20(1).jpeg' }
    ];
    ...
    const closeLightbox = () => { lightbox.classList.remove('is-open'); lightbox.setAttribute('aria-hidden', 'true'); lastImageTrigger?.focus(); };
    const moveLightbox = direction => { activeImageIndex = (activeImageIndex + direction + reviewImages.length) % reviewImages.length; renderLightbox(); };
    const openLightbox = trigger => {
        const index = reviewImages.findIndex(image => image.url === trigger.dataset.reviewImage);
        if (index < 0) return;
        ...
        setTimeout(() => lightbox.querySelector('.review-image-lightbox__close').focus(), 20);
    };
    const reviewMarkup = comment => {
        ...
        const image = imageUrl ? `<button class="review-card__image-button" type="button" data-review-image="${escapeHtml(imageUrl)}" aria-label="Yorum görselini büyüt"><img class="review-card__image" src="${escapeHtml(imageUrl)}" alt="${escapeHtml(imageAlt)}" loading="lazy"><span>Görseli incele <b aria-hidden="true">↗</b></span></button>` : '';
        const reply = String(comment.admin_reply || '').trim();
        const replyMarkup = reply ? `<aside class="review-card__reply"><span>Pioneer yanıtı</span><p>${escapeHtml(reply)}</p></aside>` : '';
        return `<article class="review-card${imageUrl ? ' review-card--has-image' : ''}">${image}<div class="review-card__identity"><span class="review-card__initial">${escapeHtml(initials(comment.full_name))}</span><div><strong class="review-card__name">${escapeHtml(comment.full_name)}</strong><p class="review-card__meta">${escapeHtml(comment.location || 'Tekirdağ')} · ${rating}</p></div></div><p class="review-card__text">${escapeHtml(comment.body)}</p>${replyMarkup}</article>`;
    };
