...
if ($authenticated) { try { $pdo = db(); $counts = dueCounts($pdo); $cards = summary($pdo); if ($tab === 'comments') $comments = $pdo->query('SELECT * FROM comments ORDER BY CASE status WHEN "pending" THEN 0 WHEN "approved" THEN 1 ELSE 2 END, created_at DESC')->fetchAll(); if (in_array($tab, ['payables', 'receivables'], true)) { $flow = $tab === 'payables' ? 'expense' : 'income'; $statement = $pdo->prepare('SELECT * FROM finance_entries WHERE payment_type = "pending" AND status IN ("pending", "partial") AND flow = ? ORDER BY CASE WHEN due_date IS NULL THEN 1 ELSE 0 END, due_date, created_at DESC'); $statement->execute([$flow]); $pendingEntries = $statement->fetchAll(); } if ($tab === 'finance') $ledger = ledger($pdo); } catch (Throwable $exception) { error_log('Pioneer admin database: ' . $exception->getMessage()); $flash = ['type' => 'error', 'text' => 'Veritabanı başlatılamadı.']; } }
?>
<!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><title>Pioneer Yönetim Paneli</title><link rel="stylesheet" href="admin-v4.css"></head><body class="admin-body <?= $authenticated ? 'is-authenticated' : 'is-guest' ?>">
