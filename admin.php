...
function db(): PDO {
    ...
    $pdo->exec('CREATE TABLE IF NOT EXISTS finance_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, flow TEXT NOT NULL CHECK(flow IN ("income", "expense")), payment_type TEXT NOT NULL CHECK(payment_type IN ("instant", "pending")), amount_cents INTEGER NOT NULL CHECK(amount_cents > 0), remaining_cents INTEGER NOT NULL CHECK(remaining_cents >= 0), due_date TEXT, note TEXT NOT NULL, status TEXT NOT NULL CHECK(status IN ("completed", "pending", "partial", "cancelled")), created_at TEXT NOT NULL, updated_at TEXT NOT NULL)');
    $pdo->exec('CREATE TABLE IF NOT EXISTS finance_payments (id INTEGER PRIMARY KEY AUTOINCREMENT, entry_id INTEGER NOT NULL, amount_cents INTEGER NOT NULL CHECK(amount_cents > 0), note TEXT, paid_at TEXT NOT NULL, created_at TEXT NOT NULL, FOREIGN KEY(entry_id) REFERENCES finance_entries(id) ON DELETE CASCADE)');
    $pdo->exec('CREATE TABLE IF NOT EXISTS finance_audit (id INTEGER PRIMARY KEY AUTOINCREMENT, entry_id INTEGER, action TEXT NOT NULL, payload TEXT, created_at TEXT NOT NULL)');
    $pdo->exec('CREATE INDEX IF NOT EXISTS idx_finance_entries_status ON finance_entries(status, due_date)');
    $pdo->exec('CREATE INDEX IF NOT EXISTS idx_finance_payments_entry ON finance_payments(entry_id, paid_at DESC)');
    return $pdo;
}
...
function summary(PDO $pdo): array {
    $actual = $pdo->query('SELECT flow, COALESCE(SUM(amount_cents), 0) total FROM (SELECT flow, amount_cents FROM finance_entries WHERE payment_type = "instant" AND status = "completed" UNION ALL SELECT e.flow, p.amount_cents FROM finance_payments p JOIN finance_entries e ON e.id = p.entry_id) GROUP BY flow')->fetchAll();
    $cash = ['income' => 0, 'expense' => 0]; foreach ($actual as $row) $cash[$row['flow']] = (int) $row['total'];
    $month = (new DateTimeImmutable('now', new DateTimeZone('Europe/Istanbul')))->format('Y-m');
    $monthly = $pdo->prepare('SELECT flow, COALESCE(SUM(amount_cents), 0) total FROM (SELECT flow, amount_cents, created_at FROM finance_entries WHERE payment_type = "instant" AND status = "completed" UNION ALL SELECT e.flow, p.amount_cents, p.paid_at AS created_at FROM finance_payments p JOIN finance_entries e ON e.id = p.entry_id) WHERE strftime("%Y-%m", created_at) = ? GROUP BY flow');
    $monthly->execute([$month]); $monthCash = ['income' => 0, 'expense' => 0]; foreach ($monthly->fetchAll() as $row) $monthCash[$row['flow']] = (int) $row['total'];
    ...
    return ['balance' => $cash['income'] - $cash['expense'], 'income' => $cash['income'], 'expense' => $cash['expense'], 'month_income' => $monthCash['income'], 'month_expense' => $monthCash['expense'], 'awaiting_income' => $awaiting['income'], 'awaiting_expense' => $awaiting['expense']];
}
function ledger(PDO $pdo): array {
    $sql = 'SELECT created_at, flow, amount_cents, note, "instant" AS entry_kind, id AS source_id FROM finance_entries WHERE payment_type = "instant" AND status = "completed" UNION ALL SELECT p.paid_at AS created_at, e.flow, p.amount_cents, COALESCE(NULLIF(p.note, ""), e.note) AS note, "settlement" AS entry_kind, e.id AS source_id FROM finance_payments p JOIN finance_entries e ON e.id = p.entry_id ORDER BY datetime(created_at) DESC';
...
if ($authenticated && $_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'comment_review') {
            ...
            if (mb_strlen($reply) > 1200) throw new InvalidArgumentException('İşletme yanıtı en fazla 1200 karakter olabilir.');
            $update = $pdo->prepare('UPDATE comments SET status = ?, admin_reply = ?, replied_at = CASE WHEN ? <> "" THEN ? ELSE replied_at END, reviewed_at = ? WHERE id = ?');
            $update->execute([$status, $reply ?: null, $reply, now(), now(), $id]); flash('success', $status === 'approved' ? 'Yorum onaylandı ve yayın akışına alındı.' : 'Yorum reddedildi.'); redirect('comments');
        }
        ...
        if ($action === 'finance_create') {
            $flow = $_POST['flow'] ?? ''; $paymentType = $_POST['payment_type'] ?? ''; $amount = cents($_POST['amount'] ?? ''); $note = trim((string) ($_POST['note'] ?? '')); $due = trim((string) ($_POST['due_date'] ?? ''));
            ...
            if ($paymentType === 'pending' && (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $due) || $due < (new DateTimeImmutable('today'))->format('Y-m-d'))) throw new InvalidArgumentException('Bekleyen ödeme için geçerli bir tarih seçin.');
            $status = $paymentType === 'instant' ? 'completed' : 'pending'; $time = now();
            $insert = $pdo->prepare('INSERT INTO finance_entries (flow, payment_type, amount_cents, remaining_cents, due_date, note, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
            $insert->execute([$flow, $paymentType, $amount, $paymentType === 'pending' ? $amount : 0, $paymentType === 'pending' ? $due : null, $note, $status, $time, $time]);
            $id = (int) $pdo->lastInsertId(); audit($pdo, $id, 'created', compact('flow', 'paymentType', 'amount', 'due', 'note'));
        ...
        if ($action === 'finance_edit') {
            ...
            if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $due)) throw new InvalidArgumentException('Geçerli bir vade tarihi seçin.');
            $remaining = $amount - $paid; $status = $remaining === 0 ? 'completed' : ($paid > 0 ? 'partial' : 'pending');
            $pdo->prepare('UPDATE finance_entries SET amount_cents = ?, remaining_cents = ?, note = ?, due_date = ?, status = ?, updated_at = ? WHERE id = ?')->execute([$amount, $remaining, $note, $due, $status, now(), $id]); audit($pdo, $id, 'edited', compact('amount', 'remaining', 'note', 'due'));
            flash('success', 'Bekleyen ödeme güncellendi.'); redirect($entry['flow'] === 'income' ? 'receivables' : 'payables');
        }
        if ($action === 'finance_settle') {
            $id = filter_var($_POST['entry_id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]); $amount = cents($_POST['amount'] ?? ''); $note = trim((string) ($_POST['note'] ?? ''));
            ...
            $pdo->prepare('INSERT INTO finance_payments (entry_id, amount_cents, note, paid_at, created_at) VALUES (?, ?, ?, ?, ?)')->execute([$id, $amount, $note ?: null, $time, $time]);
