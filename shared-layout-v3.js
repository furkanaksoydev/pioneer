(() => {
    ...
    // The home page keeps its original header controller; only its construction header needs this shared controller.
    const headers = page === 'home'
        ? document.querySelectorAll('[data-shared-shell-header="construction"]')
        : document.querySelectorAll('[data-shared-shell-header]');
    ...
    headers.forEach(header => {
        ...
        const scrollRoot = header.closest('.construction-open-view');
        const scrollTarget = scrollRoot || window;
        const scrollState = () => header.classList.toggle('is-scrolled', (scrollRoot ? scrollRoot.scrollTop : window.scrollY) > 32);
        scrollState();
        scrollTarget.addEventListener('scroll', scrollState, { passive: true });
    });
})();
